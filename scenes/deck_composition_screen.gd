...
func create_card_display(card_key: String) -> void:
	...
	card_instance.assign_data(card_key)
	...
	for card_key in selected_cards:
		if CardDatabase.CARDS.has(card_key):
			...
			for i in range(count):
				deck_data.append({
					"card_name": card_key,
					"data": CardDatabase.CARDS[card_key]
				})
	return deck_data


# Save the current deck composition (can be extended to save to file)
func save_deck(deck_name: String = "default") -> void:
	var deck_data = get_selected_deck()
	print("Saving deck '%s' with %d cards" % [deck_name, deck_data.size()])
	# TODO: Implement actual save functionality to file or game state


# Load a previously saved deck
func load_deck(deck_name: String = "default") -> void:
	# TODO: Implement actual load functionality
	print("Loading deck: ", deck_name)
