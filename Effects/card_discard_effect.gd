...
## Effect that discards target card(s) and optionally previews them before discarding
...
## Whether to show a visual preview of the discarded card
...
func apply_effect(context: GameContext) -> void:
	if not target_strategy:
		push_error("CardDiscardEffect: target_strategy is not set")
		return

	var targets = target_strategy.select_targets(context)

	if targets.is_empty():
		print("  → No targets found to discard")
		return
...
func _build_command_chain(target: CardData, context: GameContext) -> void:
	if not target or not target.strategy:
		push_error("CardDiscardEffect: Invalid target CardData")
		return
	...
	# Final command: Add to discard pile
	var add_to_pile_cmd = AddCardToPileCommand.new(GameEnums.PileType.DISCARD)
	...
	print("  → Card discard chain queued: ", target.strategy.card_id)
